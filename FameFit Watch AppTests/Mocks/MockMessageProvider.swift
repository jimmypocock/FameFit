...
public class MockMessageProvider: MessageProviding {
    ...
    public func setMockResponses(
        ...
        if let message { mockMessage = message }
        if let timeAware { mockTimeAwareMessage = timeAware }
        if let motivational { mockMotivationalMessage = motivational }
        if let roast { mockRoastMessage = roast }
        if let catchphrase { mockCatchphrase = catchphrase }
    }
    ...
    public func assertPersonalityUpdated(
        ...
        XCTAssertEqual(
            lastUpdate.roastLevel,
            expectedPersonality.roastLevel,
            "Roast level mismatch",
            file: file,
            line: line
        )
        XCTAssertEqual(
            lastUpdate.includeHumblebrags,
            expectedPersonality.includeHumblebrags,
            "Humblebrags setting mismatch",
            file: file,
            line: line
        )
        XCTAssertEqual(
            lastUpdate.includeSocialMediaRefs,
            expectedPersonality.includeSocialMediaRefs,
            "Social media refs setting mismatch",
            file: file,
            line: line
        )
    }
