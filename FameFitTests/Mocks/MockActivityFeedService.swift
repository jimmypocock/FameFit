...
final class MockActivityFeedService: ActivityFeedProtocol {
  ...
  func fetchFeed(for userIDs: Set<String>, since: Date?, limit: Int) async throws
    -> [ActivityFeedRecord]
  {
    if shouldFail {
      throw mockError
    ...
    if postedActivities.isEmpty, !userIDs.isEmpty {
      ...
      let baseTimeOffset: TimeInterval =
        if let since {
      ...
      for (index, userID) in userIDs.prefix(3).enumerated() {  // Create activities for up to 3 users
        ...
        // Workout activity
        let workoutContent = ActivityFeedItemContent(
        ...
        // Level up activity
        let levelUpContent = ActivityFeedItemContent(
