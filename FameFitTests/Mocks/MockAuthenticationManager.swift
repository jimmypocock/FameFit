...
//  FameFitTests
//
//  Mock implementation of AuthenticationManaging for testing
//
...
class MockAuthenticationManager: AuthenticationManaging {
    ...
    func signOut() {
        ...
        if shouldFailSignOut {
            lastError = .unknownError(NSError(domain: "MockAuthError", code: 4, userInfo: [NSLocalizedDescriptionKey: "Sign out failed"]))
            return
        }
    ...
    func completeOnboarding() {
        ...
        if shouldFailOnboarding {
            lastError = .unknownError(NSError(domain: "MockAuthError", code: 5, userInfo: [NSLocalizedDescriptionKey: "Onboarding failed"]))
            return
        }
    ...
    func simulateAuthenticationError(_ error: FameFitError = .authenticationFailed(NSError(domain: "SimulatedAuthError", code: 3))) {
