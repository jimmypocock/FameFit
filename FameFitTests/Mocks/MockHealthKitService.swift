...
final class MockHealthKitService: HealthKitService {
    ...
    var anchoredQueryHandler: ((HKAnchoredObjectQuery, [HKSample]?, [HKDeletedObject]?, HKQueryAnchor?, Error?)
        -> Void
    )?
    var anchoredQueryUpdateHandler: ((HKAnchoredObjectQuery, [HKSample]?, [HKDeletedObject]?, HKQueryAnchor?, Error?)
        -> Void
    )?
    var anchoredQueryInitialHandler: ((HKAnchoredObjectQuery, [HKSample]?, [HKDeletedObject]?, HKQueryAnchor?, Error?)
        -> Void
    )?
    ...
    func startObservingWorkouts(
        ...
        workoutUpdateHandler = updateHandler
    ...
    func fetchWorkoutsWithPredicate(
        ...
        DispatchQueue.main.async {
                ...
                if predicateString.contains("startDate") || predicateString.contains("endDate") {
                    ...
                    workoutsToReturn = workoutsToReturn.compactMap { sample -> HKSample? in
                        ...
                        let appInstallDate = UserDefaults.standard.object(forKey: appInstallDateKey) as? Date ?? Date
                            .distantPast
                        let lastProcessedDate = UserDefaults.standard
                            .object(forKey: lastProcessedKey) as? Date ?? appInstallDate
            ...
            if !sortDescriptors.isEmpty {
                workoutsToReturn = (workoutsToReturn as NSArray)
                    .sortedArray(using: sortDescriptors) as? [HKSample] ?? []
            }
    ...
    func createWorkoutSession(configuration _: HKWorkoutConfiguration) -> HKWorkoutSession? {
    ...
    func executeAnchoredQuery(
        ...
        anchoredQueryInitialHandler = initialHandler
        anchoredQueryUpdateHandler = updateHandler
    ...
    func simulateInitialAnchoredQueryResults(workouts: [HKWorkout]) {
        guard let handler = anchoredQueryInitialHandler,
              let query = activeQueries.first(where: { $0 is HKAnchoredObjectQuery }) as? HKAnchoredObjectQuery
        else {
            return
        }
