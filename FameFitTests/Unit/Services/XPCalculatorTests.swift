...
final class XPCalculatorTests: XCTestCase {
    ...
    func testBasicXPCalculation() {
        ...
        let workout = createWorkout(duration: 3_600, workoutType: "Running", startDate: specificDate, averageHeartRate: nil)
    ...
    func testStreakMultipliers() {
        ...
        for testCase in testCases {
            ...
            XCTAssertEqual(xp, expectedXP, "\(testCase.streak) day streak should have \(testCase.multiplier)x multiplier")
        }
    }
    ...
    func testSpecialBonuses() {
        ...
        // Holiday bonus
        XCTAssertEqual(XPCalculator.calculateSpecialBonus(workoutNumber: 5, isPersonalRecord: false, isHoliday: true), 50)
        
        // Combined bonuses
        XCTAssertEqual(XPCalculator.calculateSpecialBonus(workoutNumber: 10, isPersonalRecord: true, isHoliday: true), 175)
    }
    ...
    func testLevelCalculation() {
        ...
        for testCase in testCases {
            ...
            XCTAssertEqual(levelInfo.level, testCase.expectedLevel, "XP \(testCase.xp) should be level \(testCase.expectedLevel)")
            XCTAssertEqual(levelInfo.title, testCase.expectedTitle, "XP \(testCase.xp) should have title '\(testCase.expectedTitle)'")
        }
    }
    ...
    func testNextLevelXP() {
        ...
        for testCase in testCases {
            let levelInfo = XPCalculator.getLevel(for: testCase.xp)
            XCTAssertEqual(levelInfo.nextLevelXP, testCase.expectedNext, "XP \(testCase.xp) should need \(testCase.expectedNext) for next level")
        }
    }
