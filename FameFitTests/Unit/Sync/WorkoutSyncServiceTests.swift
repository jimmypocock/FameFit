...
// Simple test cases for WorkoutSyncService focusing on key behaviors
class WorkoutSyncServiceTests: XCTestCase {
    ...
    func testStartReliableSync_WhenHealthKitAvailable_NoError() {
        ...
        // Then
        // Note: We can't fully test the anchored query behavior because WorkoutSyncService
    ...
    // The following tests would require refactoring WorkoutSyncService to use
    ...
    func testProcessWorkouts_AddsFollowersForEachWorkout() async {
        ...
        let maxRetries = 10
        var retries = 0
        while mockCloudKitService.addXPCalls.count < 3, retries < maxRetries {
            try? await Task.sleep(nanoseconds: 100_000_000) // 0.1 seconds
            retries += 1
        }

        // Then - Should have called addXP for each workout
        XCTAssertEqual(mockCloudKitService.addXPCalls.count, 3)
        // Each workout should earn XP based on duration and type (not fixed 5)
        XCTAssertTrue(mockCloudKitService.addXPCalls.allSatisfy { $0.xp > 0 })
    }
    ...
    func testProcessWorkouts_SkipsPreInstallWorkouts() {
        ...
        XCTAssertEqual(mockCloudKitService.addXPCalls.count, 1)
    ...
    func testInitialSync_DoesNotSendNotifications() {
        ...
        XCTAssertEqual(mockCloudKitService.addXPCalls.count, 0)
    ...
    func testIncrementalSync_SendsNotifications() {
        ...
        XCTAssertEqual(mockCloudKitService.addXPCalls.count, 2)
        XCTAssertEqual(mockNotificationStore.notifications.count, 2)
    }
