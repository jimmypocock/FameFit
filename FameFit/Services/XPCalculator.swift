...
struct XPCalculator {
    ...
    static func getLevel(for totalXP: Int) -> (level: Int, title: String, nextLevelXP: Int) {
        ...
        for i in 0..<levels.count {
            if i == levels.count - 1 || totalXP < levels[i + 1].threshold {
                let currentLevel = i + 1
                let currentTitle = levels[i].title
                let nextLevelXP = i < levels.count - 1 ? levels[i + 1].threshold : Int.max
...
extension XPCalculator {
    static let unlockables: [XPUnlock] = [
        ...
        // Features
        XPUnlock(xpRequired: 100, name: "Custom Messages", description: "Personalized workout messages", category: .feature),
        XPUnlock(xpRequired: 1_000, name: "Workout Stats", description: "Detailed analytics dashboard", category: .feature),
        XPUnlock(xpRequired: 5_000, name: "Character Personality", description: "New coach personality options", category: .feature),
        XPUnlock(xpRequired: 25_000, name: "Custom App Icon", description: "Exclusive app icons", category: .feature),
        XPUnlock(xpRequired: 100_000, name: "Exclusive Workouts", description: "Special workout types", category: .feature),
        
        // Customization
        XPUnlock(xpRequired: 500, name: "Profile Theme", description: "Customize your profile look", category: .customization),
        XPUnlock(xpRequired: 2_500, name: "Workout Themes", description: "Custom workout UI themes", category: .customization),
        XPUnlock(xpRequired: 10_000, name: "Animation Pack", description: "Special celebration animations", category: .customization),
        
        // Achievements
        XPUnlock(xpRequired: 1_000, name: "Fitness Enthusiast", description: "Reached 1,000 XP!", category: .achievement),
        XPUnlock(xpRequired: 10_000, name: "Rising Star", description: "Reached 10,000 XP!", category: .achievement),
        XPUnlock(xpRequired: 100_000, name: "FameFit Elite", description: "Reached 100,000 XP!", category: .achievement),
        XPUnlock(xpRequired: 1_000_000, name: "FameFit God", description: "Reached 1 million XP!", category: .achievement)
    ]
    
    static func getAvailableUnlocks(for xp: Int) -> [XPUnlock] {
        return unlockables.filter { $0.xpRequired <= xp }
    }
