...
class WorkoutSyncQueue: ObservableObject, WorkoutSyncQueueProtocol {
    ...
    func processQueue() {
        ...
        guard let cloudKitManager, cloudKitManager.isAvailable else {
    ...
    private func processPendingWorkouts(_ workouts: [PendingWorkout]) {
        ...
        for workout in workouts {
            ...
            if let lastRetry = workout.lastRetryDate {
                ...
                if timeSinceLastRetry < retryDelay {
                    FameFitLogger.debug(
                        "Skipping workout \(workout.id) - retry delay not met",
            ...
            if syncSuccess {
                ...
                if retryWorkout.retryCount < maxRetries {
                    ...
                    FameFitLogger.notice(
                        "Will retry workout \(workout.id) later (attempt \(retryWorkout.retryCount)/\(maxRetries))",
                        category: FameFitLogger.workout
                    )
                } else {
                    FameFitLogger.error(
                        "Failed to sync workout \(workout.id) after \(maxRetries) attempts",
                        category: FameFitLogger.workout
                    )
                }
            }
        ...
        DispatchQueue.main.async {
            ...
            FameFitLogger.info(
                "Sync complete: \(successCount) succeeded, \(failureCount) failed, \(workoutsToRetry.count) pending retry",
                category: FameFitLogger.workout
            )
        }
    ...
    private func loadQueue() {
        ...
        do {
            ...
            FameFitLogger.info(
                "Loaded \(pendingWorkouts.count) pending workouts from storage",
                category: FameFitLogger.workout
            )
        } catch {
    ...
    func isWorkoutInQueue(_ workout: HKWorkout) -> Bool {
        ...
        pendingWorkouts.contains { pending in
