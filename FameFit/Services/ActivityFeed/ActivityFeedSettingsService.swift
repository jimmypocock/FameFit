...
final class ActivityFeedSettingsService: ActivityFeedSettingsProtocol {
    ...
    func loadSettings() async throws -> ActivityFeedSettings {
        // Check if we have a user ID - if not, return cached settings or defaults
        guard cloudKitManager.currentUserID != nil else {
            ...
            // Return defaults if no cached settings and no user ID
            let defaultSettings = ActivityFeedSettings(userID: "")
