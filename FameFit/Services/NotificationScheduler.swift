...
struct NotificationRequest {
    ...
    init(
        ...
        self.sound = type.soundEnabled ? .default : nil
    }
}
...
final class NotificationScheduler: NotificationScheduling {
    ...
    private func addToBatch(_ notification: NotificationRequest) {
        ...
        if batchTimers[notification.type] == nil {
            let timer = Timer.scheduledTimer(withTimeInterval: TimeInterval(preferences.batchingWindowMinutes * 60), repeats: false) { _ in
                Task {
    ...
    private func createGroupedNotification(from notifications: [NotificationRequest], type: NotificationType) -> NotificationRequest {
