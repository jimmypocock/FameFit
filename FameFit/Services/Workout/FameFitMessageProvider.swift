...
public class FameFitMessageProvider: MessagingProtocol {
    ...
    public func getMotivationalMessage() -> String {
        getRandomMessage(from: .encouragement)
    }
    ...
    public func getCatchphrase() -> String {
        getRandomMessage(from: .catchphrases)
    }
    ...
    private func getRandomWorkoutMessage(for context: MessageContext) -> String {
        if shouldShowRoast() {
            getRoastMessage(for: context.workoutType)
        } else {
    ...
    private func getWorkoutSpecificRoast(for workoutType: HKWorkoutActivityType) -> String {
        ...
        case .running, .walking:
            runningRoasts.randomElement() ?? getRandomMessage(from: .roast)
        case .traditionalStrengthTraining, .functionalStrengthTraining:
            strengthRoasts.randomElement() ?? getRandomMessage(from: .roast)
        default:
            getRandomMessage(from: .roast)
        }
    }
    ...
    public func getWorkoutEndMessage(workoutType: String, duration: Int, calories: Int, xpEarned: Int) -> String {
        ...
        let baseMessage = getRandomMessage(from: .workoutEnd)
        return "\(baseMessage) You earned \(xpEarned) XP!"
    }
    
    public func getStreakMessage(streak: Int, isAtRisk: Bool) -> String {
        if isAtRisk {
            if shouldShowRoast() {
                return "⚠️ Yo, your \(streak) day streak is on LIFE SUPPORT! Don't be the person who fumbles at the finish line!"
            } else {
                return "⚠️ Don't lose your \(streak) day streak! Get moving today!"
            }
        } else {
            return "🔥 \(streak) day streak! That's what I'm talking about! ELITE behavior right there!"
        }
    }
    
    public func getXPMilestoneMessage(level: Int, title: String) -> String {
        return "🎉 LEVEL \(level) UNLOCKED: \(title)! You're literally becoming a LEGEND! #BuiltDifferent"
    }
    
    public func getFollowerMessage(username: String, displayName: String, action: String) -> String {
        return "📱 \(displayName) just \(action)! Your influence is GROWING! That's the power of CONSISTENCY!"
    }
}
