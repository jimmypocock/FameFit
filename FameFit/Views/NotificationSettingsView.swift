...
struct NotificationSettingsView: View {
            ...
            Form {
                ...
                if preferences.pushNotificationsEnabled {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        if preferences.quietHoursEnabled {
                            DatePicker(
                                "Start Time",
                                selection: Binding(
                                    get: { preferences.quietHoursStart ?? Calendar.current.date(bySettingHour: 22, minute: 0, second: 0, of: Date())! },
                            ...
                            DatePicker(
                                "End Time", 
                                selection: Binding(
                                    get: { preferences.quietHoursEnd ?? Calendar.current.date(bySettingHour: 8, minute: 0, second: 0, of: Date())! },
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Stepper("Max per Hour: \(preferences.maxNotificationsPerHour)", 
                               value: $preferences.maxNotificationsPerHour, 
                               in: 1...60)
                        
                        Stepper("Max per Day: \(preferences.maxNotificationsPerDay)", 
    ...
    private func checkPushNotificationStatus() async {
        ...
        await MainActor.run {
            self.pushNotificationStatus = status
        }
    }
