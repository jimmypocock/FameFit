...
protocol ActivityFeedCommentWithUserProtocol {
    associatedtype CommentType: CommentProtocol
...
struct AnyComment: CommentProtocol {
    let id: String
    let userID: String
    let content: String
    ...
    init<T: CommentProtocol>(_ comment: T) {
        self.id = comment.id
...
class AnyCommentService: CommentServiceProtocol {
    ...
    init<Service: CommentServiceProtocol>(_ service: Service) {
